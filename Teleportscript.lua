...
local function getFirstRail()
	while true do
		...
		task.wait(0.001) -- Reduced wait for faster checking
	end
end
...
-- This loop waits for the initial firstRail
while not firstRail and task.wait(0.001) do -- Reduced wait for faster checking
...
local function scanFor(func, ...)
	...
	local rail
	while task.wait(0.001) do -- Reduced wait for faster checking
...
task.spawn(function()
	while task.wait(0.01) do -- This loop updates references, 0.01s is likely fine here
...
local function teleportToChair(cchair, dontChangeChair, inLoop)
	...
	if inLoop then
		repeat
			task.wait(0.1) -- This wait is inside the teleportToChair loop, might need adjustment based on desired sitting speed
...
function teleport(position, y, z, posName)
	...
	if dist < 50 then
		...
		-- This loop is for short distance movement, faster checks might be okay
		for i=1, ((dist / plr.Character.Humanoid.WalkSpeed / 2) - 1) * 100 do
			plr.Character.Humanoid:MoveTo(position.Position)
			task.wait(0.001) -- Reduced wait for faster movement steps
		end
		if teleports.Values then
	...
	elseif dist < 50 then
		...
		-- This loop is for moving to the chair, faster checks might be okay
		for i=1, ((dist / plr.Character.Humanoid.WalkSpeed / 2) - 1) * 100 do
	...
	repeat
		...
		for i=1, 5 do
			...
			task.wait(0.001) -- Reduced wait for faster teleport steps
		end
		...
		-- The wait inside teleportToChair (0.1) might still be a factor in this loop's speed
...
teleports.Teleports = table.freeze({
	...
	End = function(self)
		...
		-- Reduced wait for faster checking
		repeat task.wait(0.001) until workspace.Baseplates:FindFirstChild("FinalBasePlate") and workspace.Baseplates.FinalBasePlate:FindFirstChild("OutlawBase")
