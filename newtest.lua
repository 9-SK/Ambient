...
local viridianValues = {
	viridianVersion = "1.26",
	viridianName = "viridian",
	releaseType = "Stable",
	viridianFolder = "viridian",
	...
	pingProfile = {
		...
		adaptiveBaselineSamples = 30, -- how many samples viridian takes before deciding on a fixed high ping value
...
local viridianSettings = {
	{
		name = 'General',
		description = 'The general settings for viridian, from simple to unique features.',
		color = Color3.new(0.117647, 0.490196, 0.72549),
		minimumLicense = 'Free',
		categorySettings = {
			{
				name = 'Anonymous Client',
				description = 'Randomise your username in real-time in any CoreGui parented interface, including viridian. You will still appear as your actual name to others in-game. This setting can be performance intensive.',
				settingType = 'Boolean',
				current = false,
			...
			{
				name = 'Chat Spy',
				description = 'This will only work on the legacy Roblox chat system. viridian will display whispers usually hidden from you in the chat box.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'When active, viridian will notify you when the next song in your Music queue plays.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'Cancel any kick request involving you sent by the client. This may prompt detection from Adonis or similar anti-cheats. You will need to rejoin and re-run viridian to toggle.',
				settingType = 'Boolean',
				current = false,
	...
	{
		name = 'Keybinds',
		description = 'Assign keybinds to actions or change keybinds such as the one to open/close viridian.',
		...
		categorySettings = {
			...
			},
			{
				...
				id = 'noclip',
				callback = function()
					local noclip = viridianValues.actions[1]
			...
			},
			{
				...
				id = 'flight',
				callback = function()
					local flight = viridianValues.actions[2]
			...
			},
			{
				...
				id = 'refresh',
				callback = function()
					local refresh = viridianValues.actions[3]
			...
			},
			{
				...
				id = 'respawn',
				callback = function()
					local respawn = viridianValues.actions[4]
			...
			},
			{
				...
				id = 'invulnerability',
				callback = function()
					local invulnerability = viridianValues.actions[5]
			...
			},
			{
				...
				id = 'fling',
				callback = function()
					local fling = viridianValues.actions[6]
			...
			},
			{
				...
				id = 'esp',
				callback = function()
					local esp = viridianValues.actions[7]
			...
			},
			{
				...
				id = 'nightandday',
				callback = function()
					local nightandday = viridianValues.actions[8]
			...
			},
			{
				...
				id = 'globalaudio',
				callback = function()
					local globalaudio = viridianValues.actions[9]
			...
			},
			{
				...
				id = 'visibility',
				callback = function()
					local visibility = viridianValues.actions[10]
	...
	{
		name = 'Performance',
		description = 'Tweak and test your performance settings for Roblox in viridian.',
		color = Color3.new(1, 0.376471, 0.168627),
		minimumLicense = 'Free',
		categorySettings = {
			{
				name = 'Artificial FPS Limit',
				description = 'viridian will automatically set your FPS to this number when you are tabbed-in to Roblox.',
				settingType = 'Number',
				values = {20, 5000},
				current = 240,
			...
			},
			{
				...
				description = 'viridian will automatically set your FPS to 60 when you tab-out or unfocus from Roblox.',
			...
			},
			{
				...
				description = 'viridian will check your average latency in the background and notify you if your current latency significantly goes above your average latency.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'viridian will check your average FPS in the background and notify you if your current FPS goes below a specific number.',
				settingType = 'Boolean',
				current = true,
	...
	{
		name = 'Detections',
		description = 'viridian detects and prevents anything malicious or possibly harmful to your wellbeing.',
		...
		categorySettings = {
			...
			},
			{
				...
				description = 'Be notified whenever viridian detects a player joins your session that could be a game moderator.',
				settingType = 'Boolean', 
				minimumLicense = 'Pro',
				current = true,
...
local randomAdjective = viridianValues.nameGeneration.adjectives[math.random(1, #viridianValues.nameGeneration.adjectives)]
local randomNoun = viridianValues.nameGeneration.nouns[math.random(1, #viridianValues.nameGeneration.nouns)]
...
-- Initialise viridian Client Interface
local guiParent = gethui and gethui() or coreGui
local viridian = guiParent:FindFirstChild("viridian")
if viridian then
	viridian:Destroy()
end

local UI = game:GetObjects('rbxassetid://'..viridianValues.interfaceAsset)[1]
UI.Name = viridianValues.viridianName
UI.Parent = guiParent
UI.Enabled = false
...
-- put this into viridianValues, like the fps and ping shit
...
if not getMessage then viridianValues.chatSpy.enabled = false end
...
-- viridian Functions
local function checkviridian() return UI.Parent end
local function getPing() return math.clamp(statsService.Network.ServerStatsItem["Data Ping"]:GetValue(), 10, 700) end
local function checkFolder() if isfolder then if not isfolder(viridianValues.viridianFolder) then makefolder(viridianValues.viridianFolder) end if not isfolder(viridianValues.viridianFolder.."/Music") then makefolder(viridianValues.viridianFolder.."/Music") writefile(viridianValues.viridianFolder.."/Music/readme.txt", "Hey there! Place your MP3 or other audio files in this folder, and have the ability to play them through the viridian Music UI!") end if not isfolder(viridianValues.viridianFolder.."/Assets/Icons") then makefolder(viridianValues.viridianFolder.."/Assets/Icons") end if not isfolder(viridianValues.viridianFolder.."/Assets") then makefolder(viridianValues.viridianFolder.."/Assets") end end end
...
local function fetchFromCDN(path, write, savePath)
	pcall(function()
		...
		local file = game:HttpGet(viridianValues.cdn..path) or nil
		if not file then return end
		if not write then return file end


		writefile(viridianValues.viridianFolder.."/"..savePath, file)
...
local function fetchIcon(iconName)
	pcall(function()
		...
		local pathCDN = viridianValues.icons..iconName..".png"
		local path = viridianValues.viridianFolder.."/Assets/"..iconName..".png"
...
local function createEsp(player)
	if player == localPlayer or not checkviridian() then 
		return
	end
	...
	highlight.Enabled = viridianValues.actions[7].enabled
	highlight.Parent = espContainer

	player.CharacterAdded:Connect(function(character)
		if not checkviridian() then return end
		task.wait()
...
local function checkAction(target)
	...
	for _, action in ipairs(viridianValues.actions) do
...
local function checkSetting(settingTarget, categoryTarget)
	for _, category in ipairs(viridianSettings) do
...
local function wipeTransparency(ins, target, checkSelf, tween, duration)
	local transparencyProperties = viridianValues.transparencyProperties
...
local function blurSignature(value)
	if not value then
		if lighting:FindFirstChild("viridianBlur") then
			lighting:FindFirstChild("viridianBlur"):Destroy()
		end
	else
		if not lighting:FindFirstChild("viridianBlur") then
			local blurLight = Instance.new("DepthOfFieldEffect", lighting)
			blurLight.Name = "viridianBlur"
...
local function checkLastVersion()
	...
	local lastVersion = isfile and isfile(viridianValues.viridianFolder.."/".."version.srs") and readfile(viridianValues.viridianFolder.."/".."version.srs") or nil

	if lastVersion then
		if lastVersion ~= viridianValues.viridianVersion then queueNotification("viridian has been updated", "viridian has been updated to version "..viridianValues.viridianVersion..", check our Discord for all new features and changes.", 4400701828)  end
	end

	if writefile then writefile(viridianValues.viridianFolder.."/".."version.srs", viridianValues.viridianVersion) end
end
...
local function removeReverbs(timing)
	...
	for index, sound in next, soundInstances do
		if sound:FindFirstChild("viridianAudioProfile") then
			local reverb = sound:FindFirstChild("viridianAudioProfile")
...
local function playNext()
	...
	local asset = getcustomasset(viridianValues.viridianFolder.."/Music/"..musicQueue[1].sound)
...
local function addToQueue(file)
	...
	checkFolder()
	if not isfile(viridianValues.viridianFolder.."/Music/"..file) then queueNotification("Unable to locate file", "Please ensure that your audio file is in the viridian/Music folder and that you are including the file extension (e.g mp3 or ogg).", 4370341699) return end
	...
	getLength.SoundId = getcustomasset(viridianValues.viridianFolder.."/Music/"..file)
	getLength.Volume = 0
	getLength:Play()
	task.wait(0.05)
...
local function createReverb(timing)
	for index, sound in next, soundInstances do
		if not sound:FindFirstChild("viridianAudioProfile") then
			local reverb = Instance.new("EqualizerSoundEffect")

			reverb.Name = "viridianAudioProfile"
			reverb.Parent = sound
...
local function syncExperienceInformation()
	viridianValues.currentCreator = creatorId

	if creatorType == Enum.CreatorType.Group then
		viridianValues.currentGroup = creatorId
		viridianValues.currentCreator = "group"
	end

	for _, gameFound in pairs(viridianValues.games) do
		if gameFound.id == placeId and gameFound.enabled then
			...
			local rawFile = viridianValues.rawTree..gameFound.raw
			viridianValues.currentGame = gameFound
			...
			gameDetectionPrompt.Layer.Run.MouseButton1Click:Connect(function()
				closeGameDetection()
				queueNotification("Running "..gameFound.name, "Now running viridian' "..gameFound.name.." script, this may take a moment.", 4400701828)
				runScript(rawFile)
...
local function updateSliderPadding()
	for _, v in pairs(viridianValues.sliders) do
		v.padding = {
...
local function sortActions()	
	...
	for _, action in ipairs(viridianValues.actions) do
		...
		newAction.Interact.MouseButton1Click:Connect(function()
			...
			if not success then
				queueNotification("Action Error", "This action ('"..(action.name).."') had an error while running, please report this to the viridian team at getviridan.com/discord", 4370336704)
				action.enabled = false
				newAction.Icon.Image = "rbxassetid://"..action.images[2]
				tweenService:Create(newAction, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.55}):Play()
				tweenService:Create(newAction.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 0.4}):Play()
	...
	if localPlayer.Character then
		if not localPlayer.Character:FindFirstChildOfClass('Humanoid').UseJumpPower then
			viridianValues.sliders[2].name = "jump height"
			viridianValues.sliders[2].default = 7.2
			viridianValues.sliders[2].values = {0, 120}
		end
	end


	for _, slider in ipairs(viridianValues.sliders) do
		...
		newSlider.Interact.MouseButton1Down:Connect(function()
			if debounce or not checkviridian() then return end
...
local function getAdaptiveHighPingThreshold()
	local adaptiveBaselinePings = viridianValues.pingProfile.adaptiveBaselinePings

	if #adaptiveBaselinePings == 0 then
		return viridianValues.pingProfile.adaptiveHighPingThreshold
	end
	...
	return median * viridianValues.pingProfile.spikeThreshold
end

local function checkHighPing()
	local recentPings = viridianValues.pingProfile.recentPings
	local adaptiveBaselinePings = viridianValues.pingProfile.adaptiveBaselinePings

	local currentPing = getPing()
	table.insert(recentPings, currentPing)

	if #recentPings > viridianValues.pingProfile.maxSamples then
		table.remove(recentPings, 1)
	end

	if #adaptiveBaselinePings < viridianValues.pingProfile.adaptiveBaselineSamples then
...
local function closePanel(panelName, openingOther)
	...
	if not openingOther then
		...
		tweenService:Create(panel, TweenInfo.new(0.65, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {Position = viridianValues.buttonPositions[panelName]}):Play()
...
local function openPanel(panelName)
	...
	panel.Position = viridianValues.buttonPositions[panelName]
	...
	if panel.Name == "Character" then -- Character Panel Animation
		...
		for _, sliderV in pairs(sliderInfo) do
			if characterPanel.Interactions.Sliders:FindFirstChild(sliderV[1]) then
				...
				for _, sliderFound in ipairs(viridianValues.sliders) do
...
local function UpdateHome()
	...
	if not table.find(viridianValues.executors, string.lower(identifyexecutor())) then
...
local function securityDetection(title, content, link, gradient, actions)
	...
	checkFolder()
	local currentAllowlist = isfile and isfile(viridianValues.viridianFolder.."/".."allowedLinks.srs") and readfile(viridianValues.viridianFolder.."/".."allowedLinks.srs") or nil
			...
			if action[2] then
				if action[3] then
					...
					if currentAllowlist then
						...
						writefile(viridianValues.viridianFolder.."/".."allowedLinks.srs", httpService:JSONEncode(currentAllowlist))
					else
						writefile(viridianValues.viridianFolder.."/".."allowedLinks.srs", httpService:JSONEncode({domain}))
					end
				end
				decision = true
			else
...
if Essential or Pro then
	getgenv()[index] = function(data)
		if checkviridian() and checkSetting("Intelligent HTTP Interception").current then
			local title = "Do you trust this source?"
			local content = "viridian has prevented data from being sent off-client, would you like to allow data to be sent or retrieved from this source?"
			...
			if url == "http://127.0.0.1:6463/rpc?v=1" then
				...
				if bodyDecoded.cmd == "INVITE_BROWSER" then
					...
					content = "viridian has prevented your Discord client from automatically joining this Discord server, would you like to continue and join, or block it?"
	...
	getgenv()[indexSetClipboard] = function(data)
		if checkviridian() and checkSetting("Intelligent Clipboard Interception").current then
			local title = "Would you like to copy this to your clipboard?"
			local content = "viridian has prevented a script from setting the below text to your clipboard, would you like to allow this, or prevent it from copying?"
...
local function onChatted(player, message)
	local enabled = checkSetting("Chat Spy").current and viridianValues.chatSpy.enabled
	local chatSpyVisuals = viridianValues.chatSpy.visual

	if not message or not checkviridian() then return end
	...
	if enabled and player ~= localPlayer then
		...
		if hidden and enabled then
			chatSpyVisuals.Text = "viridian Spy - [".. player.Name .."]: "..message2
...
local function saveSettings()
	...
	if isfile and isfile(viridianValues.viridianFolder.."/"..viridianValues.settingsFile) then
		writefile(viridianValues.viridianFolder.."/"..viridianValues.settingsFile, httpService:JSONEncode(viridianSettings))
	end
end
...
local function assembleSettings()
	if isfile and isfile(viridianValues.viridianFolder.."/"..viridianValues.settingsFile) then
		...
		local success, response = pcall(function()
			currentSettings = httpService:JSONDecode(readfile(viridianValues.viridianFolder.."/"..viridianValues.settingsFile))
		end)

		if success then
			for _, liveCategory in ipairs(viridianSettings) do
			...
			writefile(viridianValues.viridianFolder.."/"..viridianValues.settingsFile, httpService:JSONEncode(viridianSettings)) -- Update file with any new settings added
		end
	else
		if writefile then
			checkFolder()
			if not isfile(viridianValues.viridianFolder.."/"..viridianValues.settingsFile) then
				writefile(viridianValues.viridianFolder.."/"..viridianValues.settingsFile, httpService:JSONEncode(viridianSettings))
			end
		end 
	end

	for _, category in viridianSettings do
			...
			if not setting.hidden then
				...
				elseif settingType == "Input" then
					...
					newInput.InputFrame.InputBox.FocusLost:Connect(function()
						if minimumLicense then
							if (minimumLicense == "Pro" and not Pro) or (minimumLicense == "Essential" and not (Pro or Essential)) then
								queueNotification("This feature is locked", "You must be "..minimumLicense.." or higher to use "..setting.name..". \n\nUpgrade at https://getviridan.com", 4483345875)
								newInput.InputFrame.InputBox.Text = setting.current
								return
							end
						end
	...
	if checkSetting("Client-Based Anti Kick").current then
		if hookmetamethod then 
			...
			originalIndex = hookmetamethod(game, "__index", function(self, method)
				if self == localPlayer and method:lower() == "kick" and checkSetting("Client-Based Anti Kick").current and checkviridian() then
					queueNotification("Kick Prevented", "viridian has prevented you from being kicked by the client.", 4400699701)
			...
			originalNamecall = hookmetamethod(game, "__namecall", function(self, ...)
				if self == localPlayer and getnamecallmethod():lower() == "kick" and checkSetting("Client-Based Anti Kick").current and checkviridian() then
					queueNotification("Kick Prevented", "viridian has prevented you from being kicked by the client.", 4400699701)
					return
				end
...
local function start()
	if viridianValues.releaseType == "Experimental" then -- Make this more secure.
		if not Pro then localPlayer:Kick("This is an experimental release, you must be Pro to run this. \n\nUpgrade at https://getviridan.com/") return end
	end
	...
	if not checkSetting("Load Hidden").current then 
		...
		--	local startupPath = viridianValues.viridianFolder.."/Assets/startup.wav"
		--	local startupAsset
	...
	if script_key and not (Essential or Pro) then
		queueNotification("License Error", "We've detected a key being placed above viridian loadstring, however your key seems to be invalid. Make a support request at getviridan.com/discord to get this solved within minutes.", "document-minus")
	end

	if viridianValues.enableExperienceSync then
...
homeContainer.Interactions.Discord.Interact.MouseButton1Click:Connect(function()
	if setclipboard then 
		originalSetClipboard("getviridan.com/discord")
		queueNotification("Discord Invite Copied", "We've set your clipboard to the viridian discord invite.", 4335479121)
	else
...
userInputService.InputEnded:Connect(function(input, processed)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 then
		for _, slider in pairs(viridianValues.sliders) do
			slider.active = false

			if characterPanel.Visible and not debounce and slider.object and checkviridian() then
...
players.PlayerAdded:Connect(function(player)
	...
	if checkSetting("Moderator Detection").current and Pro then
		...
		if viridianValues.currentCreator == "group" then
			for _, role in pairs(viridianValues.administratorRoles) do 
				if string.find(string.lower(roleFound), role) then
					promptModerator(player, roleFound)
					queueNotification("Administrator Joined", viridianValues.currentGroup .." "..roleFound.." ".. player.DisplayName .." has joined your session", 3944670656) -- change to group name
				end
			end
		end
	end
...
runService.RenderStepped:Connect(function(frame)
	...
	table.insert(viridianValues.frameProfile.fpsQueue, fps)
	viridianValues.frameProfile.totalFPS += fps

	if #viridianValues.frameProfile.fpsQueue > viridianValues.frameProfile.fpsQueueSize then
		viridianValues.frameProfile.totalFPS -= viridianValues.frameProfile.fpsQueue[1]
		table.remove(viridianValues.frameProfile.fpsQueue, 1)
	end
end)
...
runService.Stepped:Connect(function()
	...
	if character then
		-- No Clip
		local noclipEnabled = viridianValues.actions[1].enabled
		local flingEnabled = viridianValues.actions[6].enabled
...
runService.Heartbeat:Connect(function()
	...
	if primaryPart then
		...
		-- Fly
		if viridianValues.actions[2].enabled then
			...
			tweenService:Create(bodyVelocity, tweenInfo, { Velocity = velocity * viridianValues.sliders[3].value * 45 }):Play()
			bodyVelocity.Parent = primaryPart

			if not viridianValues.actions[6].enabled then
...
while task.wait(1) do
	...
	if disconnectedRobloxUI and not promptedDisconnected then
		...
		for _, preDisconnectType in ipairs(viridianValues.disconnectTypes) do
	...
	if Pro then
		...
		if checkHighPing() then
			if viridianValues.pingProfile.pingNotificationCooldown <= 0 then
				if checkSetting("Adaptive Latency Warning").current then
					...
					viridianValues.pingProfile.pingNotificationCooldown = 120
				end
			end
		end

		if viridianValues.pingProfile.pingNotificationCooldown > 0 then
			viridianValues.pingProfile.pingNotificationCooldown -= 1
		end

		-- Adaptive frame time checks
		if viridianValues.frameProfile.frameNotificationCooldown <= 0 then
			if #viridianValues.frameProfile.fpsQueue > 0 then
				local avgFPS = viridianValues.frameProfile.totalFPS / #viridianValues.frameProfile.fpsQueue

				if avgFPS < viridianValues.frameProfile.lowFPSThreshold then
					if checkSetting("Adaptive Performance Warning").current then
						...
						viridianValues.frameProfile.frameNotificationCooldown = 120	
					end
				end
			end
		end
		...
		if viridianValues.frameProfile.frameNotificationCooldown > 0 then
			viridianValues.frameProfile.frameNotificationCooldown -= 1
		end
	end
end